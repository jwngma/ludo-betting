...
class AllInOnepaymentScreen extends StatefulWidget {
  final int price;
  final int coins;

  const AllInOnepaymentScreen({Key key, this.price, this.coins})
      : super(key: key);

  @override
  _AllInOnepaymentScreenState createState() => _AllInOnepaymentScreenState();
}

class _AllInOnepaymentScreenState extends State<AllInOnepaymentScreen> {
  ...
  void generateTxnToken(int mode, ProgressDialog progressDialog) async {
    ...
    try {
      ...
      await http.post(
        ...
        await AllInOneSdk.startTransaction(mid, orderId, amount.toString(),
                txnToken, null, isStaging, restrictAppInvoke)
            .then((value) {
          print(value);
