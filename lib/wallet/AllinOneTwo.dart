...
class PaymentScreenTwo extends StatefulWidget {
  ...
  const PaymentScreenTwo({Key key, this.price, this.coins}) : super(key: key);

  @override
  _PaymentScreenTwoState createState() => _PaymentScreenTwoState();
}
  ...
  Future<void> _startTransaction() async {
    ...
    try {
      var response = AllInOneSdk.startTransaction(
          mid, orderId, amount.toString(), tko, null, testing, restrictAppInvoke);
      response.then((value) {
        print(value);
        setState(() {
        ...
        if (onError is PlatformException) {
          setState(() {
            payment_response = onError.message + " \n  " + onError.details.toString();
          });
        } else {
          setState(() {
            payment_response = onError.toString();
          });
        }
      });
    } catch (err) {
